...
def deobfuscate_and_decompress(data, h):
    boff, plen = h["body_off"], h["packed_size"]
    # clamp body end to file size (some archives carry an extra tail)
    bend = min(boff + plen, len(data))
    region = bytearray(data[boff:bend])
    plen = len(region)
    # first 16 bytes are obfuscated
    if plen >= 16:
        region[:16] = _xor16(region[:16])
    # NOTE: second obfuscated 16-byte window sits at packed_size-32 (4-aligned),
    # NOT at the very end (mirrors klensy/wt-tools Obfs32Adapter)
    if plen >= 32:
        off2 = (plen - 32) // 4 * 4
        region[off2:off2 + 16] = _xor32(region[off2:off2 + 16])
    payload = bytes(region)
    max_out = max(h["orig_size"] + 4096, 1 << 20)
    if zstandard is not None:
        dctx = zstandard.ZstdDecompressor()
        obj = dctx.decompressobj()
        out = obj.decompress(payload)
        out += obj.flush() if hasattr(obj, "flush") else b""
        used = len(payload) - len(obj.unused_data) if obj.eof else None
        return out, (boff + used if used is not None else None)
    raw = _zstd.ZstdDecompressor().stream_reader(payload)  # stdlib path
    out = raw.read(max_out)
    return out, bend if raw.eof else None
...
class Vromfs:
    ...
    def read_file(self, i):
        ...
        if name == "nm":
            out = self._dict_dctx().decompress(raw[40:], max_output_size=dsize * 40 + 65536)
            return out
